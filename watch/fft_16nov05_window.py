...
#            self[REF_LEVEL_KEY] = 450
        self[REF_LEVEL_KEY] = 0.9 * max_level
...
#        samples = numpy.concatenate((samples[istep2:], samples[:istep2]))
        samples = numpy.concatenate(allsamples[istep2:istep], allsamples[0:istep2])
        ...
        if not self.once:
            ...
            self.once = True
            print 'Special block: ',self.iblock
...
#        istop2 = istart + istep2 + 1
        istop2 = istart + istep2
# stop all processing of the rest of the data
#        nblock = 1
...
#            samples = numpy.concatenate((allsamples[istart2:(istop-1)], allsamples[(istart+1):istop2]))
#            samples = allsamples[istart:istop]
            if iii == self.iblock:
                sumsamples = samples
                nblock = 1
                break
...
#           print "dt, dt/2: ", dt.total_seconds(), dtover2.total_seconds()
            ...
            print self.loopSpectrum[510:514]
            if self.plottype == PLOTLOG:
